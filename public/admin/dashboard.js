...
function toggleArea(id) {
  ...
  // Show/hide the password generator button only when add-user panel is open
  const pwBtn = document.getElementById('pw-gen-btn');
  if (pwBtn) {
    if (!isOpen && id === 'add-user') {
      pwBtn.style.display = 'inline-flex';
    } else {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  addUserForm.addEventListener('submit', async function (e) {
    ...
    const dataToSend = {
      ...
      id_number: idNumber, phone, gender, role, status, qualification
    };
